...
void MuonDTLocalMillepedeAlgorithm::terminate(void)
{
  ...
  if(workingmode == 1) {
    ...
    DTMuonMillepede myMillepede(ntuplePath, numberOfRootFiles, ptMax, ptMin, f);
 } 
  f->Write();
  f->Close();
}
...
void MuonDTLocalMillepedeAlgorithm::setBranchTrees() {
  ...
  ttreeOutput->Branch("nhits", nhits, "nhits[nseg]/F");
  ...
  ttreeOutput->Branch("ex", zc, "ex[nseg][14]/F");
