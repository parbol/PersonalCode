...
void MassSolver::gradient(double* ETmiss, double* b, double* bb, double* lp, double* lm, double mWp, double mWm, double mt, double mtb, vector<double> &grad, double step, double lambda) {
    ...
    double ETmissXPlus[2]  = {ETmissX + step * ETmissX, ETmissY};
    double ETmissXMinus[2] = {ETmissX - step * ETmissX, ETmissY};
    double ETmissYPlus[2]  = {ETmissX, ETmissY + step * ETmissY};
    double ETmissYMinus[2] = {ETmissX, ETmissY - step * ETmissY};
    ...
    //std::cout << "X plus: " << costXPlus << " X minus: " << costXMinus << " X plus: " << costYPlus << " X minus: " << costYMinus << std::endl;
    //std::cout << "Step: " << step << std::endl;
    //std::cout << "ETmissX: " << ETmissX << std::endl;
    //std::cout << "ETmissY: " << ETmissY << std::endl;
    grad.push_back(((costXPlus - costXMinus) / (2.0*step*ETmissX)));
    grad.push_back(((costYPlus - costYMinus) / (2.0*step*ETmissY)));

    double norm = sqrt(grad[0]*grad[0] + grad[1]*grad[1]); 
    grad[0] = -lambda*(grad[0]/norm);
    grad[1] = -lambda*(grad[1]/norm);
                          ...
                          double mW1, double mW2, double mt1, double mt2)
{
    ...
    double lp[4] = { lep1.E() , lep1.Px() , lep1.Py() , lep1.Pz() };
    double lm[4] = { lep2.E() , lep2.Px() , lep2.Py() , lep2.Pz() };
    return cost(ETmiss, b, bb, lp, lm, mW1, mW2, mt1, mt2);
...
// cout << a1 <<" "<<a2 <<" "<<a3 <<" "<<a4 <<" "<<c22 <<" "<<c21 <<" "<<c11 <<" "<<c20 <<" "<<c10 <<" "<<c00 <<" "<<b1 <<" "<<b2 <<" "<<b3 <<" "<<b4 <<" "<<dp22<<" "<<dp21<<" "<<dp11<<" "<<dp20<<" "<<dp10<<" "<<dp00<<endl;
  ...
  //publication formulae
    ...
    polx[0] = polx[0]/polx[4];
    polx[1] = polx[1]/polx[4];
    polx[2] = polx[2]/polx[4];
    polx[3] = polx[3]/polx[4];
    polx[4] = 1.0;
    ...
    vector<double> polx2(3);
    polx3[0] = polx3[1];
    polx3[1] = 2.0 * polx3[2];
    polx3[2] = 3.0 * polx3[3];

    //for(unsigned int i = 0; i < 4; i++) {
    //   std::cout << polx3[i] << std::endl;
    //}
    ...
    long double minDist = 1e250;
    for(unsigned int i = 0; i < pnuxt.size(); i++) {
        long double val = fabs(polx[0] + polx[1] * pnuxt[i] + polx[2] * pnuxt[i] * pnuxt[i] +  polx[3] * pnuxt[i] * pnuxt[i] * pnuxt[i] + polx[4] * pnuxt[i] * pnuxt[i] * pnuxt[i] * pnuxt[i]);
        if(val < minDist) {
            minDist = val;
        }
    } 
